...
- 底部桌宠全程陪同，随主题切换为小海豹、小猫、机器人或像素小鸡；每次翻页更新本页讲解，点击桌宠可重讲。
...
四套桌宠精灵图同样使用内置 ImageGen 按原创角色提示生成，保存在 `assets/pets/`。详见[素材说明与提示词](assets/pets/README.md)。每张包含待机、说话、指向和开心四个姿势，以透明 PNG 和 CSS 帧切换呈现。
...
网站由服务器 Caddy 的 `/study-committee/` 子路径提供，站点目录 `/srv/study-committee`。更新前备份原目录，再上传 `index.html`、`style.css`、`themes.css`、`theme-picker.css`、`pet.css`、`app.js`、`pet.js` 与 `assets/`。不需要重启后端服务，也不需要改动其他站点路由。回滚时恢复备份目录。不要将服务器凭据放进仓库。
