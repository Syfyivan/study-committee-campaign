...
一套七页的像素田园风网页演示。以原创山谷、书屋和木框手札营造轻松的竞选氛围，正文保持清晰可读。
...
- 浏览器打印可输出七页横向内容，建议开启背景图形。

## 内容与素材

姓名为宋如一，班级待填写。认真、耐心、负责是竞选态度与行动提案，不是已核实的成绩或获奖事实。正文在 `index.html`，可编辑资料在 `app.js` 的 `profile`。
...
`BASE_URL` 可切换验证地址；`SCREENSHOT_DIR` 可保存逐页截图。验证覆盖六种视口下的七页内容、溢出、键盘与滑动翻页、快速连续转场、深链接、昼夜、暂停记忆、减少动态效果、全屏、打印可见性以及浏览器/资源错误。
